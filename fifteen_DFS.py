...
# 状态节点
class State(object):
    def __init__(self, deepth=0, state=None, value=None, father=None):
        '''
        初始化
...
def A_start(start, end, generate_child_fn):
    ...
    node_hash_set.add(root.value)
    while len(OPEN) != 0:
        top = heapq.heappop(OPEN)
        if top == end_state:  # 结束后直接输出路径
            return print_path(top)
        # 产生孩子节点，孩子节点加入OPEN表
        generate_child_fn(top, end_state,node_hash_set,OPEN)
    print("无搜索路径!")  # 没有路径
    return -1

if __name__ == '__main__':
    print("有界深度优先算法（A*）解决十五数码问题")
    time1 = time.time()
    length = A_start(S0, SG, generate_child)
    time2 = time.time()
    if length != -1:
        print("最优路径一共需要"+str(length)+"步")
